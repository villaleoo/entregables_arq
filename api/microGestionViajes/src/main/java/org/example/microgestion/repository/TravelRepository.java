...
public interface TravelRepository extends JpaRepository<Viaje,Long> {
    ...
    @Query("SELECT new org.example.microgestion.DTO.externals.monopatin.ReportTravelsMonoDTO( COUNT(v),COALESCE(SUM(v.time),0), COALESCE(SUM(p.minPause), 0) )" +
            "FROM Viaje v " +
            "LEFT JOIN v.pauses p " +
            "WHERE v.id_monopatin = :id")
    Optional<ReportTravelsMonoDTO> findDetailsTravelsOfMonopatin(String id);

    @Query("SELECT new org.example.microgestion.DTO.travels.TravelDTO(v.id_travel,v.id_user,v.id_account,v.id_monopatin,v.id_stop_init,v.id_stop_end,v.date_init,v.date_end,v.kmsMonoInit,v.kmsTraveled,v.time,v.state,v.total_pay,v.isPause,v.fee,v.feeActive,v.quantityEndPauses)  FROM Viaje v")
